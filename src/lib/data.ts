...
import { Ticket, Customer, Rule, DbSupportCase, DbCustomer, DbCaseNote, DbSlaRule, DbArticle, DbChatSession, ChatMessage, TicketStatus } from "./types";
import { supabase } from "./supabase";
import { v4 as uuidv4 } from "uuid";
import { sendEmail, generateResolutionEmail } from "./email";
...
// Map DB to App types (moved outside function for reuse)
...
export function useTickets() {
  ...
  const fetchTickets = useCallback(async () => {
    ...
    try {
      ...
      // Load notes from localStorage (no database needed)
      const ticketIds = (cases || []).map((c: any) => c.id);
      let notesMap: Record<string, any[]> = {};
      
      if (typeof window !== 'undefined') {
        try {
          const storedNotes = localStorage.getItem('starbucks_ticket_notes');
          if (storedNotes) {
            const allNotes: Record<string, any[]> = JSON.parse(storedNotes);
            // Only load notes for tickets that exist
            ticketIds.forEach((ticketId: string) => {
              if (allNotes[ticketId]) {
                notesMap[ticketId] = allNotes[ticketId];
              }
            });
          }
        } catch (err) {
      ...
      // Map DB to App types (using constants defined above)
  ...
  const updateTicket = async (id: string, changes: Partial<Ticket>) => {
    ...
    // Check if ticket is being closed/resolved
    const isClosing = changes.status === 'Closed' || changes.status === 'Resolved';
    let ticketData: any = null;
    let customerEmail: string | null = null;
    let customerName: string = 'Valued Customer';

    if (isClosing) {
      // Fetch ticket and customer data for email
      ...
      if (ticket) {
        ...
        if (customer?._users) {
          customerEmail = customer._users.email;
          customerName = `${customer._users.first_name || ''} ${customer._users.last_name || ''}`.trim() || 'Valued Customer';
        }
      }
    }
    ...
    if (isClosing) {
      updates.resolution_time = Math.floor((Date.now() - new Date(ticketData?.created_at || Date.now()).getTime()) / 1000 / 60); // in minutes
    ...
    if (!error) {
      ...
      if (isClosing && ticketData && customerEmail) {
        try {
          const emailContent = generateResolutionEmail({
            customerName,
            ticketId: id,
            ticketTitle: ticketData.title || 'Support Request',
            issueDescription: ticketData.description || ticketData.title || 'Your support request',
            resolutionNotes: ticketData.internal_notes || undefined,
  ...
  const addNote = async (ticketId: string, text: string, internal: boolean = true) => {
    ...
    // Store notes in localStorage instead of database (for demo purposes)
    if (typeof window === 'undefined') {
    ...
    try {
      ...
      const storedNotes = localStorage.getItem('starbucks_ticket_notes');
      const allNotes: Record<string, any[]> = storedNotes ? JSON.parse(storedNotes) : {};
      
      // Add new note to the ticket's notes array
      if (!allNotes[ticketId]) {
        allNotes[ticketId] = [];
      }
      allNotes[ticketId].unshift(newNote); // Add to beginning (newest first)
      
      // Save back to localStorage
      localStorage.setItem('starbucks_ticket_notes', JSON.stringify(allNotes));
      ...
      console.error('Failed to add note to localStorage:', err);
      throw new Error(`Failed to save note: ${err?.message || 'Unknown error'}`);
    }
  };
  ...
  return { tickets, loading, addTicket, updateTicket, addNote, deleteTicket, refreshTickets };
}
...
export function useRules() {
    ...
    const load = async () => {
      ...
      if (data) {
        setRules(data.map((r: any) => {
          // Map actual database schema to app format
          // Your schema: sla_id, priority_level, response_time (timestamp), resolution_time (timestamp)
          // We need to convert timestamps to minutes or use them as-is
          
          // If response_time and resolution_time are timestamps representing target deadlines,
          // we'll calculate minutes from created_at (or use a default calculation)
          // For now, let's assume they represent durations stored as timestamps
          // We'll extract minutes from the timestamp difference
          
          let responseMins = 60; // Default
          let resolutionMins = 480; // Default
          
          // If response_time/resolution_time are timestamps, calculate duration
          if (r.response_time) {
            const responseTime = new Date(r.response_time).getTime();
            const createdTime = r.created_at ? new Date(r.created_at).getTime() : Date.now();
            responseMins = Math.round((responseTime - createdTime) / (1000 * 60));
            // If negative or too large, use default
            if (responseMins < 0 || responseMins > 10000) responseMins = 60;
          }
          
          if (r.resolution_time) {
            const resolutionTime = new Date(r.resolution_time).getTime();
            const createdTime = r.created_at ? new Date(r.created_at).getTime() : Date.now();
            resolutionMins = Math.round((resolutionTime - createdTime) / (1000 * 60));
            // If negative or too large, use default
            if (resolutionMins < 0 || resolutionMins > 100000) resolutionMins = 480;
          }
          
          return {
            id: r.sla_id || r.id, // Use sla_id as primary key
            name: `SLA Rule for ${r.priority_level || 'General'}`, // Generate name from priority_level
            responseMins: responseMins,
            resolutionMins: resolutionMins,
            conditionField: 'priority', // Always 'priority' since we have priority_level
            conditionValue: r.priority_level || undefined // Map priority_level to conditionValue
          };
        }));
      }
    };
  ...
  const addRule = async (name: string, responseMins: number, resolutionMins: number, conditionField?: string, conditionValue?: string) => {
    ...
    // Map to your actual database schema (NO 'name' column):
    // sla_id (uuid, auto-generated)
    // priority_level (varchar) - this is conditionValue
    // response_time (timestamp) - calculate from created_at + responseMins
    // resolution_time (timestamp) - calculate from created_at + resolutionMins
    // created_at (timestamp, auto-generated)
    
    const now = new Date();
    const responseTime = new Date(now.getTime() + (responseMins * 60 * 1000));
    const resolutionTime = new Date(now.getTime() + (resolutionMins * 60 * 1000));
    
    const insertData: any = {
      // NO 'name' column - we'll generate it from priority_level when loading
      priority_level: conditionValue || 'General', // Map conditionValue to priority_level
    ...
    try {
      const { data, error } = await client
        .from('sla_rules')
        .insert(insertData)
        .select('sla_id, priority_level, response_time, resolution_time, created_at')
        .single();
      ...
      if (data) {
        // Map back to app format (generate name from priority_level since no name column)
        const newRule: Rule = {
          id: data.sla_id,
          name: name.trim() || `SLA Rule for ${data.priority_level || 'General'}`, // Use provided name or generate
          responseMins: responseMins, // Keep original value
          resolutionMins: resolutionMins, // Keep original value
          conditionField: 'priority', // Always 'priority'
          conditionValue: data.priority_level
        };
  ...
  const updateRule = async (id: string, updates: { responseMins?: number; resolutionMins?: number; conditionValue?: string }) => {
    const client = supabase;
    if (!client) {
      throw new Error('Database connection not available');
    }
    ...
    if (updates.responseMins !== undefined) {
      const now = new Date();
      updateData.response_time = new Date(now.getTime() + (updates.responseMins * 60 * 1000)).toISOString();
    }
    
    if (updates.resolutionMins !== undefined) {
      const now = new Date();
      updateData.resolution_time = new Date(now.getTime() + (updates.resolutionMins * 60 * 1000)).toISOString();
    }
    
    if (updates.conditionValue !== undefined) {
      updateData.priority_level = updates.conditionValue;
    }
    ...
    try {
      ...
      if (data) {
        // Recalculate minutes from timestamps
        let responseMins = updates.responseMins ?? 60;
        let resolutionMins = updates.resolutionMins ?? 480;
        
        if (data.response_time && data.created_at) {
          const responseTime = new Date(data.response_time).getTime();
          const createdTime = new Date(data.created_at).getTime();
          responseMins = Math.round((responseTime - createdTime) / (1000 * 60));
        }
        
        if (data.resolution_time && data.created_at) {
          const resolutionTime = new Date(data.resolution_time).getTime();
          const createdTime = new Date(data.created_at).getTime();
          resolutionMins = Math.round((resolutionTime - createdTime) / (1000 * 60));
        }
        ...
        const updatedRule: Rule = {
          id: data.sla_id,
          name: `SLA Rule for ${data.priority_level || 'General'}`,
          responseMins: responseMins,
          resolutionMins: resolutionMins,
