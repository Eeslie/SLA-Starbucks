...
export const TicketCard: React.FC<TicketCardProps & { onClick?: () => void }> = ({ ticket, agents, onUpdate, onAddNote, onClick, rules = [] }) => {
  ...
  // 1. Find matching rule based on condition_field and condition_value (priority matching)
  const rule = rules.find(r => {
    if (r.conditionField === 'priority' && r.conditionValue) {
      return r.conditionValue.toLowerCase() === ticket.priority.toLowerCase();
    }
    // Fallback: try to match by rule name containing priority
    return r.name.toLowerCase().includes(ticket.priority.toLowerCase());
  }) || rules.find(r => !r.conditionField) || rules[0]; // Fallback to rule without condition or first rule
    ...
    <article className="sb-card p-4 hover:border-[var(--sb-green)] cursor-pointer group relative overflow-hidden" onClick={onClick}>
      <header className="flex items-start justify-between gap-3 mb-2">
        ...
        <span className={`flex-shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full border ${
          ...
          ticket.priority === 'Low' ? 'bg-blue-50 text-blue-700 border-blue-100' :
