...
export default function SlaPage() {
  ...
  const [editForm, setEditForm] = useState({ responseMins: 60, resolutionMins: 480, conditionValue: "Medium" });
  ...
  const handleAddRule = async () => {
    ...
    try {
      await addRule(newRule.name, newRule.responseMins, newRule.resolutionMins, "priority", newRule.conditionValue);
      setNewRule({ name: "", responseMins: 60, resolutionMins: 480, conditionValue: "Medium" });
    } catch (error: any) {
      console.error('Failed to add rule:', error);
      alert(`Failed to create SLA policy: ${error?.message || 'Unknown error'}`);
    }
  };
  ...
  const stats = useMemo(() => {
    ...
    const ruleStats = rules.map(rule => {
       const matchingTickets = tickets.filter(t => {
           // Only count open/in-progress tickets for active cases
           if (t.status !== 'Open' && t.status !== 'In Progress') {
             return false;
           }
           
           // Explicit Condition Logic - match by priority
           if (rule.conditionField === 'priority' && rule.conditionValue) {
               return t.priority.toLowerCase() === rule.conditionValue.toLowerCase();
           }
           
           // Fallback: If no condition, try to match by rule name containing priority
                                     ...
                                     <button
                                       onClick={() => {
                                         ...
                                         setEditForm({
                                           ...
                                           conditionValue: rule.conditionValue || 'Medium'
                                         });
                                       }}
                            ...
                            {isEditing ? (
                              <div className="mt-3 space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                  <div>
                                    <label className="text-xs font-semibold text-gray-600 mb-1 block">Response (min)</label>
                                    <input
                                      type="number"
                                      className="sb-input w-full text-sm"
                                      value={editForm.responseMins}
                                      onChange={e => setEditForm({ ...editForm, responseMins: Number(e.target.value) })}
                                    />
                                  </div>
                                  <div>
                                    <label className="text-xs font-semibold text-gray-600 mb-1 block">Resolution (min)</label>
                                    <input
                                      type="number"
                                      className="sb-input w-full text-sm"
                                      value={editForm.resolutionMins}
                                      onChange={e => setEditForm({ ...editForm, resolutionMins: Number(e.target.value) })}
                                    />
                                  </div>
                                </div>
                                    ...
                                    onClick={async () => {
                                      try {
                                        await updateRule(rule.id, {
                                          responseMins: editForm.responseMins,
                                          resolutionMins: editForm.resolutionMins,
                                          conditionValue: editForm.conditionValue
                                        });
                                        setEditingRule(null);
                                      } catch (error: any) {
               ...
               <div className="space-y-4">
                  ...
                  <div>
                     ...
                     <p className="text-xs text-gray-400 mt-1">
                        Rules match tickets based on priority. Create separate rules for each priority level.
                     </p>
                  </div>
