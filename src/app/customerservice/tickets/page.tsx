...
export default function TicketsPage() {
  const { tickets, updateTicket, addNote, deleteTicket, refreshTickets } = useTickets();
                                                                  ...
                                                                  {(() => {
                                                                    const matchingRule = rules.find(r => {
                                                                      if (r.conditionField === 'priority' && r.conditionValue) {
                                                                        return r.conditionValue.toLowerCase() === t.priority.toLowerCase();
                                                                      }
                                                                      return r.name.toLowerCase().includes(t.priority.toLowerCase());
                                                                    }) || rules[0];
                                                                    if (matchingRule && (t.status === 'Open' || t.status === 'In Progress')) {
                                                                      const created = new Date(t.createdAt).getTime();
                                                                      const targetTime = created + (matchingRule.resolutionMins * 60000);
                                                                      const now = Date.now();
      ...
      {selectedTicket && (
        <TicketDetailModal 
          ...
          onAddNote={async (id, text) => {
            await addNote(id, text, true);
            // Refresh tickets to get updated notes
            setTimeout(() => refreshTickets(), 500);
          }}
...
const PriorityBadge = ({ priority, mini = false }: { priority: string, mini?: boolean }) => {
    ...
    return (
        <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${
                isUrgent ? 'bg-red-700 shadow-sm shadow-red-300' :
                isHigh ? 'bg-red-500 shadow-sm shadow-red-200' : 
                isMed ? 'bg-amber-400' : 
                isLow ? 'bg-blue-400 shadow-sm shadow-blue-200' :
                'bg-gray-300'
            }`} />
            <span className={`text-sm font-medium ${
                isUrgent ? 'text-red-800' :
                isHigh ? 'text-red-700' : 
                isMed ? 'text-amber-700' :
                isLow ? 'text-blue-700' :
                'text-gray-600'
            }`}>{priority}</span>
        </div>
